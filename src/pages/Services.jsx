  ...
  return (
    <ul className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      {repairServices.map((item, i) => (
        <li
          key={item.name}
          className="group relative flex items-center gap-4 overflow-hidden border border-line bg-panel px-5 py-4 transition-all duration-300 hover:-translate-y-0.5 hover:border-amber-strong/60 hover:bg-amber/[0.05]"
        >
          {/* amber edge light reveals on hover / keyboard focus */}
          <span
            aria-hidden="true"
            className="absolute inset-y-0 left-0 w-[3px] bg-amber opacity-0 transition-opacity duration-300 group-focus-within:opacity-100 group-hover:opacity-100"
          />
          <span className="tnum flex h-9 w-9 shrink-0 items-center justify-center border border-line bg-panel2 font-mono text-xs font-semibold text-amber transition-colors duration-300 group-hover:border-amber-strong/60">
            {String(i + 1).padStart(2, '0')}
          </span>
          <span className="flex-1 text-[15px] font-medium text-label">{item.name}</span>
          {item.tag && (
            <span className="shrink-0 border border-line bg-panel2 px-2 py-0.5 text-[11px] font-semibold uppercase tracking-[0.08em] text-amber transition-colors duration-300 group-hover:border-amber-strong/40">
              {item.tag}
            </span>
          )}
        </li>
